...
const doiExpression = ({
  ...
  IO.map(({ webPage, pdf, publisher, journal }) => pipe(
    [
      ...
      RDF.quad(publisher, rdfs.label, RDF.literal(data.publisher), sciety('publishers')),
    ],
    D.fromArray,
    ...
    data.journal ? D.union(D.fromArray([
      ...
      RDF.quad(journal, rdf.type, fabio.Journal, sciety('journals')),
      RDF.quad(journal, dcterms.title, RDF.literal(data.journal), sciety('journals')),
    ])) : identity,
